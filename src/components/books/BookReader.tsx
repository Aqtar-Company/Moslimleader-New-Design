...
export default function BookReader({
  ...
  // Auto-hide toolbar: show on activity, hide after 3s
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    resetToolbarTimer();
    if (dx < 0) {
      // Swipe left → next for LTR, next for RTL (reading direction)
  ...
  return (
    <>
      ...
      >
        {/* ── Toolbar (auto-hide) ── */}
        <div
          className={`transition-all duration-300 ${toolbarVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}
        >
          <div className={`flex items-center gap-2 px-3 py-2.5 border-b ${dm ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
            ...
            {/* Dark mode toggle */}
            <button onClick={toggleDark} aria-label={dm ? 'وضع النهار' : 'وضع الليل'} title={dm ? 'وضع النهار' : 'وضع الليل'} className={btnCls}>
              {dm ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
