...
function ShopContent({ serverProducts }: { serverProducts: Product[] }) {
  ...
  const [displayCategories, setDisplayCategories] = useState(buildCategories(serverProducts));
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const fetched: Product[] = data.products ?? serverProducts;
...
/* ── Page (client wrapper) ───────────────────────────────────── */
export default function ShopPageClient({ serverProducts }: { serverProducts: Product[] }) {
  ...
  return (
    <>
      ...
      <FadeInSection>
        <Suspense>
          <ShopContent serverProducts={serverProducts} />
        </Suspense>
      </FadeInSection>
    </>
  );
