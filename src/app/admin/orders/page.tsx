...
function InvoiceModal({ order, onClose }: { order: AdminOrder; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="bg-gradient-to-l from-[#1a1a2e] to-[#2d1060] px-6 py-5 flex items-center justify-between">
          ...
          <div className="text-left">
            ...
            <p className="text-white/60 text-xs mt-0.5">
              {new Date(order.date).toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric' }) || order.date}
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 border-b border-gray-100 px-6 py-3 flex items-center justify-between">
          ...
          <span className={`text-xs font-bold border px-3 py-1.5 rounded-full ${STATUS_BADGE_COLORS[order.status] || 'bg-gray-50 border-gray-200 text-gray-600'}`}>
            {order.status}
          </span>
        </div>
        ...
        <div className="px-6 py-4 border-b border-gray-100 space-y-2">
          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#1a1a2e] flex items-center justify-center text-[#F5C518] font-black text-sm shrink-0">
              {(order.userName || 'ض').charAt(0)}
            </div>
            <div>
              <p className="text-sm font-bold text-gray-900">{order.userName || 'ضيف'}</p>
        ...
        <div className="px-6 py-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-black text-gray-900">
              {order.total.toLocaleString('ar-EG')} <span className="text-sm font-semibold text-gray-400">ج.م</span>
            </span>
          </div>
        </div>
        ...
        <div className="px-6 py-4 text-center">
          <button
            onClick={onClose}
            className="bg-[#1a1a2e] hover:bg-[#2d1060] text-white font-bold px-8 py-2.5 rounded-xl text-sm transition"
          >
            إغلاق
          </button>
        </div>
