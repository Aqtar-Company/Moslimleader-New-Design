...
export default function TareeqCallScreen({ callId, role, callType, remoteUser, offer, onEnd }: Props) {
  const { isRtl } = useLang();
  const [callState, setCallState] = useState<CallState>(role === 'callee' ? 'ringing' : 'connecting');
  ...
  async function endCall(status: 'ended' | 'rejected' | 'failed' = 'ended') {
    stopAll();
    setCallState(status);
    if (status !== 'failed') {
  ...
  // Caller: create offer, send it, then poll for answer + ICE
  const startCaller = useCallback(async () => {
    ...
    // Store offer in DB via POST already done before mounting — just poll for answer
    pollingRef.current = setInterval(async () => {
      ...
      if (call.status === 'rejected') { stopAll(); setCallState('rejected'); setTimeout(onEnd, 1500); return; }
      if (call.status === 'ended') { stopAll(); setCallState('ended'); setTimeout(onEnd, 800); return; }
      if (call.status === 'missed') { stopAll(); setCallState('ended'); setTimeout(onEnd, 800); return; }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [callId, callType, offer, onEnd, isRtl]);

  // Caller starts immediately on mount
  useEffect(() => {
  ...
  const stateLabel: Record<CallState, string> = {
    ringing: isRtl ? 'مكالمة واردة...' : 'Incoming call...',
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(160deg, #070d14 0%, #0d1a26 55%, #142030 100%)',
      }}
    >
      {/* Remote video (fills screen when video call is active) */}
      {callType === 'video' && (
      ...
      {callType === 'video' && callState === 'active' && (
        <video
          ...
          style={{ aspectRatio: '3/4', border: '2px solid rgba(255,255,255,0.3)', ...(cameraOff ? { display: 'none' } : {}) }}
        />
      )}
