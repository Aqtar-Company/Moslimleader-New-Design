...
// ترتيب الأنواع هنا يتبع ترتيب جدول التوزيع في RULES.md (تنازليًا حسب نسبة الظهور الفعلية
// كإجابة صحيحة عبر الـ24 قضية الحالية) — نفس الترتيب يظهر في شاشة "دليل الورثة".
const HEIR_TYPES = [
  {
    id: 'mother',
    name: 'أم',
    ...
    count: 10 // وارث فردي فقهيًا، وأكثر الأنواع ظهورًا كإجابة صحيحة (14/24 قضية) — انظر public/moro/RULES.md
  },
  {
    id: 'daughter',
    name: 'بنت',
    ...
    count: 10 // نسبة ظهور مساوية للأم (14/24 قضية) — انظر public/moro/RULES.md
  },
  {
    id: 'son',
    name: 'ابن',
    ...
    count: 9 // 13/24 قضية — انظر public/moro/RULES.md
  },
  {
    id: 'father',
    name: 'أب',
    ...
    count: 9 // وارث فردي فقهيًا (لا يمكن أن يوجد للمتوفى أكثر من أب واحد)، 12/24 قضية — تصادم لاعبين على نفس الدور محلول بمنطق "الأسرع" (getLateSingularClaimPlayers)
  },
  {
    id: 'wife',
    name: 'زوجة',
    ...
    count: 6 // 9/24 قضية — انظر public/moro/RULES.md
  },
  {
    id: 'husband',
    name: 'زوج',
    ...
    count: 5 // وارث فردي فقهيًا — المرأة لها زوج واحد فقط وقت وفاتها، بخلاف الزوجة (يصح تعددها حتى 4)، 7/24 قضية
  },
  {
    id: 'sister',
    name: 'أخت شقيقة',
    ...
    count: 4 // 5/24 قضية — انظر public/moro/RULES.md
  },
  {
    id: 'uncle',
    name: 'عم',
  ...
  },
  {
    ...
    count: 3 // وارث فردي فقهيًا، نادر الظهور (2/24 قضية) — حد أدنى 3 نُّسخ عشان يفضل موجود تعليميًا
  },
  {
    id: 'grandmother',
    name: 'جدة (لأم)',
    ...
    count: 3 // وارث فردي فقهيًا، نادر الظهور (2/24 قضية) — حد أدنى 3 نُّسخ
  },
  {
    id: 'brother',
    name: 'أخ شقيق',
    ...
    count: 3 // نادر الظهور (2/24 قضية) — حد أدنى 3 نُّسخ
  },
  {
    id: 'half-brother',
    name: 'أخ لأم',
    ...
    count: 3 // أندر الأنواع ظهورًا (1/24 قضية فقط) — حد أدنى 3 نُّسخ عشان ميبقاش كارت ميت مضمون
  },
  {
    id: 'half-sister',
    name: 'أخت لأم',
    ...
    count: 3 // أندر الأنواع ظهورًا (1/24 قضية فقط) — حد أدنى 3 نُّسخ
  },
  {
    id: 'joker',
    name: 'جوكر',
    ...
    count: 4 // يطابق عدد بطاقة "وارث" في النسخة المطبوعة (RULES.md) — مسموحة في كل الـ24 قضية
  },
  // ثلاثة أنواع عصبات أبعد — مُضافة لمحرك الفرائض وللدليل التفاعلي (heirs-circle.html) فقط،
  // بـ count:0 عمدًا حتى لا تُخلَط في رزمة اللعبة الرقمية الفعلية أو تؤثر على أيٍّ من الـ24 قضية
  // الحالية (اللي مفيش وحدة منها بتشير لهذه الأنواع في disallowed أصلًا).
  {
    id: 'grandson',
    name: 'ابن الابن',
    icon: '🧒',
    color: '#5A7FA6',
    description: 'ابن الابن (وإن نزل) يقوم مقام الابن تمامًا عند غيابه: عصبة، للذكر مثل حظ الأنثيين مع البنت، ويحجب الإخوة والأعمام كما يفعل الابن. يُحجب كليًا بوجود الابن نفسه.',
    count: 0
  },
  {
    id: 'nephew',
    name: 'ابن الأخ الشقيق',
    icon: '🧑‍🦰',
    color: '#6B8E6B',
    description: 'ابن الأخ الشقيق عصبة بنفسه، في مرتبة أقرب من العم في ترتيب العصبات الأبعد: يُحجب بالابن أو ابن الابن أو الأب أو الجد أو الإخوة الأشقاء، ويحجب هو بدوره العم عند وجوده.',
    count: 0
  },
  {
    id: 'cousin',
    name: 'ابن العم الشقيق',
    icon: '🧑‍🦱',
    color: '#9B6B4F',
    description: 'ابن العم الشقيق عصبة بنفسه، في آخر مرتبة معتمدة بهذا النظام المبسّط: يُحجب بكل من سبقه في ترتيب العصبات (الأبناء وبنوهم، الآباء والأجداد، الإخوة وبنوهم، العم).',
    count: 0
  }
];
...
const HEIR_QURAN_REF = {
  ...
  brother: { ayah: 'nisa-176', note: 'آخر آية في سورة النساء (آية الكلالة) — تشمل نصًّا الإخوة والأخوات عمومًا (الأشقاء ومن في حكمهم)؛ هذه البطاقة تمثّل الإخوة الأشقاء فقط ضمن التبسيط المعتمد هنا. "وإن كانوا إخوة رجالًا ونساء فللذكر مثل حظ الأنثيين".' },
