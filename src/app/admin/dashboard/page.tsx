...
export default function DashboardPage() {
  ...
  if (!stats) return <div className="flex items-center justify-center h-40"><div className="w-7 h-7 border-4 border-[#F5C518] border-t-transparent rounded-full animate-spin" /></div>;

  const cards = [
    { label: 'إجمالي الطلبات', value: stats.totalOrders, icon: '📦', color: 'bg-blue-50 border-blue-200', link: '/admin/orders' },
    { label: 'الإيرادات (EGP)', value: stats.totalRevenue.toLocaleString('ar-EG'), icon: '💰', color: 'bg-green-50 border-green-200', link: '/admin/orders' },
    { label: 'العملاء المسجلين', value: stats.totalUsers, icon: '👥', color: 'bg-purple-50 border-purple-200', link: '/admin/users' },
    { label: 'الكوبونات النشطة', value: stats.activeCoupons, icon: '🎟️', color: 'bg-amber-50 border-amber-200', link: '/admin/coupons' },
  ];
    ...
    <div className="space-y-6">
      ...
      {stats.outOfStock > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <Link href="/admin/products" className="text-xs font-bold text-red-600 underline">عرض</Link>
        </div>
      )}
              ...
              <tbody className="divide-y divide-gray-50">
                {stats.recentOrders.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-3 font-semibold text-gray-900">{o.total.toLocaleString('ar-EG')} ج</td>
