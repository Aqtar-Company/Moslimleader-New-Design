...
function CommunityCard({ memberNumber, joinedYear, qrDataUrl, isRtl }: CommunityCardProps) {
  ...
  return (
    <div style={{
      ...
      {/* BOTTOM-RIGHT GROUP: QR + separator + brand text */}
      <div style={{ position: 'absolute', bottom: 14, right: 18, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {qrDataUrl ? (
          ...
          <div style={{ width: 50, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <p style={{ fontSize: 8, color: 'rgba(255,255,255,0.12)', letterSpacing: '0.08em' }}>moslimleader.com</p>
          </div>
        )}
      </div>
    </div>
...
function LeaderCard({ memberNumber, memberSince, expiresAt, status, qrDataUrl, isRtl }: LeaderCardProps) {
  ...
  const cardBg = isInactive
    ? 'linear-gradient(135deg, #111114 0%, #1c1c22 40%, #222228 70%, #111114 100%)'
  ...
  return (
    <div style={{
      ...
      {/* TOP-RIGHT GROUP: logo + separator + membership type */}
      <div style={{ position: 'absolute', top: 14, right: 18, display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ textAlign: 'right' }}>
      ...
      <div style={{ position: 'absolute', bottom: 14, right: 18, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {qrDataUrl ? (
          ...
          <div style={{ width: 50, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <p style={{ fontSize: 8, color: 'rgba(255,255,255,0.1)', letterSpacing: '0.08em', writingMode: 'vertical-rl' }}>moslimleader.com</p>
          </div>
        )}
      </div>
    </div>
