    ...
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 print:hidden" style={navStyle}>
        ...
        >
          ...
          {/* 5 — Profile / Settings (leftmost in RTL) */}
          <Link
            href={user ? `/tareeq/u/${user.id}` : '/login?next=/tareeq'}
            className="flex flex-col items-center justify-end gap-0.5 pb-2 transition-all active:scale-90"
            style={{ minWidth: 44 }}
          >
            <svg
              className="w-6 h-6"
              fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"
              style={{ color: (pathname.startsWith('/tareeq/u/') || pathname === '/tareeq/profile') ? 'var(--tr-gold)' : 'rgba(255,255,255,0.38)', filter: (pathname.startsWith('/tareeq/u/') || pathname === '/tareeq/profile') ? 'drop-shadow(0 0 5px rgba(212,168,83,0.45))' : 'none', transition: 'all 0.2s' }}
            >
